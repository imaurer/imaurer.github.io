---
title: "PangoPup: Splice Prediction We Can Ship"
description: "A splice prediction model takes about four seconds per variant on a CPU, which does not work when every patient's variants need scoring. PangoPup compiles four billion already-published scores into a memory-mapped index that answers in 0.441 microseconds. This talk covers the index, the model fallback behind it, and the process boundary that keeps a GPL model out of a commercial product."
date: 2026-09-10
youtubeId: 1rf-sMUZiP8
category: bioinformatics
tags: [PangoPup, splicing, rust, genomics, open-source]
event: "GenomOncology engineering talk"
eventDate: 2026-09-10
summary: |
  Classifying a variant for a cancer or rare disease patient means working out what that variant does to the protein. Sometimes the answer is splicing: a mutation near the edge of an exon changes how the RNA gets cut and pasted, and a different protein comes out. Machine learning models predict this well. Running one on every variant, for every patient, is where it falls apart.

  This talk walks through PangoPup, an open-source Rust service built on the Pangolin splice model. Pangolin takes about four seconds per variant on a CPU, and GPUs are expensive. Someone had already run the model across roughly 19,000 genes and published about four billion scores to Zenodo, so PangoPup compiles that dump into a purpose-built, memory-mapped index. A covered single-base change answers with a median of 0.441 microseconds in about 12 MiB of memory. Anything the index cannot hold runs the real model on the CPU, and SQLite keeps that answer so it never runs twice.

  The second half covers licensing as an engineering problem. Pangolin is GPL, and a commercial product cannot absorb that. PangoPup puts the model behind one process boundary, reached as a CLI or an HTTP service, so nothing links across it. The talk ends on where a splice score actually goes: ACMG evidence codes, and the calibration work that stands between a raw score and a defensible classification.
takeaways:
  - "Splice prediction is a real input to variant classification. A mutation near an exon boundary can change the protein, and ACMG evidence codes depend on knowing that."
  - "The model is the easy part. Pangolin takes about four seconds per variant on a CPU, and GPU capacity is expensive, so running it per patient at scale does not work."
  - "Reuse beat recompute. Someone had already scored roughly 19,000 genes and published about four billion rows to Zenodo, which removed the need to rent GPUs at all."
  - "A purpose-built index beats a general one. Rather than SQLite or a generic key-value store, PangoPup compiles the dump into the smallest record that answers exactly one query shape: median 0.441 microseconds, about 12 MiB for a single-variant call."
  - "The fallback still has to be correct. Insertions, deletions, and lookup misses run the real model on the CPU, and SQLite stores the result so the same variant never costs a second inference."
  - "Licensing is an architecture problem. Pangolin is GPL, so PangoPup isolates it behind one process boundary reached as a CLI or an HTTP service. Nothing links across that line."
  - "Measurements come with their conditions. The numbers are warm-page-cache observations on one Linux laptop, not cross-host guarantees."
chapters:
  - { time: "00:00:00", label: "What PangoPup is" }
  - { time: "00:00:24", label: "What splice prediction actually is" }
  - { time: "00:02:40", label: "SpliceAI, and why we cannot ship it" }
  - { time: "00:03:21", label: "The Broad's lookup site does not scale" }
  - { time: "00:04:02", label: "Pangolin is open, and it is GPL" }
  - { time: "00:06:11", label: "Without a GPU, the model is slow" }
  - { time: "00:06:41", label: "One process boundary contains the GPL" }
  - { time: "00:08:01", label: "The architecture" }
  - { time: "00:08:36", label: "Someone already computed the scores" }
  - { time: "00:09:26", label: "Inside the 15 GB index" }
  - { time: "00:10:06", label: "Non-SNVs run the model once" }
  - { time: "00:10:31", label: "Why a purpose-built index beats a general one" }
  - { time: "00:12:22", label: "Four seconds to 0.441 microseconds" }
  - { time: "00:13:27", label: "The CLI and the service" }
  - { time: "00:14:15", label: "Turning a splice score into ACMG evidence" }
---
